...
We aim to partition these into \( K \) clusters \( C_1, C_2, \dots, C_K \), minimizing the following objective:
...
1. **Initialization**: Randomly select K data points as initial centroids \( \boldsymbol{\mu}_1, \dots, \boldsymbol{\mu}_K \).
2. **Assignment Step**: Assign each data point \( \mathbf{x}_i \) to the nearest centroid, forming K clusters, typically using Euclidean distance.

\[
   \text{Cluster}(\mathbf{x}_i) = \arg\min_k \| \mathbf{x}_i - \boldsymbol{\mu}_k \|
   \]
...
- `initialize_centroids(X, k)`:
  - Randomly selects \( \mu_k^{(0)} \) — the initial centroids.
- `assign_clusters(X, centroids)`:
  - Assigns each point \( \mathbf{x}_i \) to the nearest centroid \( \mu_k \), minimizing \( \| \mathbf{x}_i - \mu_k \| \).
...
The plot shows that the test data is well-aligned with the nonlinear boundary \[
x_2 = \sin(4x_1) + x_1
\]
, clearly separating the two classes across the oscillating decision curve
...
The KNN algorithm estimates the class label of a new observation \( \mathbf{x} \) by examining its \( k \) nearest neighbors in the training set and choosing the most common label among them.
...
Where:
- \( \mathcal{N}_k(\mathbf{x}) \) denotes the set of the \( k \) nearest training observations to \( \mathbf{x} \) (based on Euclidean distance).
- \( y_i \) is the class label of neighbor \( i \).
...
Where:
- \( \mathcal{N}_k(x) \) denotes the \( k \) closest training samples to test point \( x \).
...
Based on the curve, the optimal value of \( k \) is approximately:

\[
k = \arg\max_k \text{Accuracy}(k) \approx 3 \text{ to } 5
\]
